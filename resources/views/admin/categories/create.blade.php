...
<form method="POST" action="{{route('admin.categories.store')}}" enctype="multipart/form-data">
    @csrf
        <div class="mb-6">
            ...
            <input type="text" id="name" name="name"  class="  bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-blak dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('name') border-red-400 @enderror">
        </div>
        @error('name')
            <div class="text-sm text-red-400">{{ $message }}</div>
        @enderror
        <div>
    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-blak" for="image">Upload Image</label>
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-blak-300" id="file_input_help">SVG, PNG, JPG or GIF (MAX. 800x400px).</p>
        </div>
        @error('image')
        <div class="text-sm text-red-400">{{ $message }}</div>
    @enderror
        <div class="mb-5">
            <label for="description" class="block mb-3 text-sm font-medium text-gray-900 dark:text-blak">Description</label>
    ...
    @enderror
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <button type="submit" class=" px-5 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white" >SAVE CATEGORY</button>
                </div>
